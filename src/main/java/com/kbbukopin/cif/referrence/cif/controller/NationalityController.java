package com.kbbukopin.cif.referrence.cif.controller;

import java.util.List;

import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.kbbukopin.cif.referrence.cif.model.Nationality;
import com.kbbukopin.cif.referrence.cif.service.NationalityService;

import lombok.AllArgsConstructor;

@RestController
@RequestMapping(path = "nationality/")
@AllArgsConstructor
public class NationalityController {
	private final NationalityService service;
	
	@GetMapping(path = "")
	private List<Nationality> getAll(){
		return service.getAllData();
	}
	
	@GetMapping(path = "/{id}")
	private Nationality findById(@PathVariable Long id) throws Exception {
		return service.findById(id);
	}
}
